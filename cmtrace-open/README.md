...
The bootstrap companion at `scripts/collection/Invoke-CmtraceEvidenceBootstrap.ps1` stages the collector and profile under `C:\ProgramData\CmtraceOpen\Staging`, but each bootstrap or remediation run resolves its own staged collector and profile paths for that run. The staged collector remains generic, and runtime values such as the SAS URL, bundle metadata, and local-only mode are still passed through the scheduled task arguments rather than being baked into the staged file. The one-time `SYSTEM` scheduled task carries those exact staged paths for the run it registers. The collector still expects PowerShell 7.5.4, but the bootstrap now enforces that prerequisite by downloading and installing the pinned PowerShell 7.5.4 x64 MSI when `pwsh.exe` is missing or older than required. If Intune or IME logs appear to show a space inserted in the staged profile path, treat that as line wrapping or copied-output formatting unless the file on disk actually differs. The profile URL must return raw JSON; when it does not, the bootstrap now reports the staged path, source URL context, and a short payload preview to make non-JSON responses easier to identify.

For Intune Remediations, pair `scripts/collection/Detect-CmtraceEvidenceBootstrap.ps1` with `scripts/collection/Remediate-CmtraceEvidenceBootstrap.ps1`. The detection script only clears stale, invalid, expired, or task-orphaned throttle state and exits `1` when remediation should run; otherwise it exits `0` and leaves the active throttle in place. The remediation script is the self-contained upload target because Intune stores uploaded script content rather than a repo-relative file path. It now writes a remediation transcript under `C:\ProgramData\CmtraceOpen\Logs` for the bootstrap run, not the later collector scheduled task, and its normal output stays compact while including the transcript path and extra troubleshooting context.
